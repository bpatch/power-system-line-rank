...
alg4_data_k1 = np.empty((number_MC_samples, P*4))
alg4_data_k2 = np.empty((number_MC_samples, P*4))
alg4_data_k3 = np.empty((number_MC_samples, P*4))
alg4_data_k4 = np.empty((number_MC_samples, P*4))
alg4_data_k5 = np.empty((number_MC_samples, P*4))

counter = 1
counter2 = 0
np.random.seed(2020)

lambda_variable_initial = np.copy(line_limits)

nominal_net_line_flows = np.dot(vee_matrix, (np.concatenate((nominal_injections, np.zeros((1,int(number_buses-number_generators)))[0]),axis=0)+nominal_demand).reshape(-1,1))

abs_nominal_net_line_flows = np.abs(nominal_net_line_flows)
...
for number_test_samples in rangeSamples:
    for sample in range(number_MC_samples):
        ...
        line_flows_data = np.dot(vee_matrix, np.concatenate((data, np.zeros((int(number_buses-number_generators),number_test_samples.astype(int)))),axis=0)+nominal_demand.reshape(-1,1))
        line_flows_noise_data = np.empty((number_lines, number_test_samples.astype(int)))
        
        for ell in range(number_lines):
            if nominal_net_line_flows[ell][0] >= 0:
                line_flows_noise_data[ell,:] = line_flows_data[ell,:] - nominal_net_line_flows[ell]
            else:
                line_flows_noise_data[ell,:] = -(line_flows_data[ell,:] - nominal_net_line_flows[ell])
        ...
        for ell in range(number_lines):
            def y_convex_con_subproblem(lambda_variable):
                return -lambda_variable*(line_limits[ell]-abs_nominal_net_line_flows[ell]) + scipy.special.logsumexp(np.multiply(lambda_variable,line_flows_noise_data[ell]), b= 1/number_test_samples.astype(int))

            initial_condition = np.copy(lambda_variable_initial[ell])
            
            soln = scipy.optimize.minimize(y_convex_con_subproblem, initial_condition,method='SLSQP',options={'disp': False, 'maxiter': 1000})
        ...
        alg4_data_k1[sample, counter2] = max(indices_of_true_ranks_1)+1
        ...
        alg4_data_k2[sample, counter2] = max(indices_of_true_ranks_2)+1       
        ...
        alg4_data_k3[sample, counter2] = max(indices_of_true_ranks_3)+1  
        ...
        alg4_data_k4[sample, counter2] = max(indices_of_true_ranks_4)+1   
        ...
        alg4_data_k5[sample, counter2] = max(indices_of_true_ranks_5)+1  
       
        print(100*counter/total_simulation_effort, '%')
        counter += 1
    counter2 += 1

plt.plot(range(len(np.mean(alg4_data_k1 <= 1, axis=0))),1-np.mean(alg4_data_k2 <= 2, axis=0), 'r-')
np.savetxt('alg4data_k1_G_MC=1000_P=100.csv',alg4_data_k1.astype(int))
np.savetxt('alg4data_k2_G_MC=1000_P=100.csv',alg4_data_k2.astype(int))
#np.savetxt('alg4data_k3_G_MC=1000_P=100.csv',alg4_data_k3.astype(int))
#np.savetxt('alg4data_k4_G_MC=1000_P=100.csv',alg4_data_k4.astype(int))
#np.savetxt('alg4data_k5_G_MC=1000_P=100.csv',alg4_data_k5.astype(int))
