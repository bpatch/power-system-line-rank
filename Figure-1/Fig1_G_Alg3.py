...
rangeSamples = np.hstack((np.linspace(10,10**2,P), np.linspace(10**2,10**3,10),np.linspace(10**3,10**4,10),np.linspace(10**4,10**5,10)))

total_simulation_effort = number_MC_samples*P*4

alg3_data_k1 = np.empty((number_MC_samples, P*4))
alg3_data_k2 = np.empty((number_MC_samples, P*4))
alg3_data_k3 = np.empty((number_MC_samples, P*4))
alg3_data_k4 = np.empty((number_MC_samples, P*4))
alg3_data_k5 = np.empty((number_MC_samples, P*4))
                     
counter = 1
counter2 = 0

x_initial_alg3 = np.repeat([nominal_injections],repeats = number_lines, axis=0)
...
for number_test_samples in rangeSamples:
    for sample in range(number_MC_samples):
        ...
        beta_estimates = np.sum(np.abs(data-nominal_injections.reshape(-1,1)),axis=1)/number_samples
        def laplace_convex_con(x):
            return np.dot(1/beta_estimates, np.abs(x-nominal_injections))
        ...
        for ell in range(number_lines):
            ...
            def constraint(x):
                 line_flows = np.abs(np.dot(vee_matrix, (np.concatenate((x, np.zeros((1,int(number_buses-number_generators)))[0]),axis=0)+nominal_demand).reshape(-1,1)))
                 return line_flows[ell]-line_limits[ell]
            
            con = {'type': 'ineq', 'fun': constraint}
            
            initial_condition = np.copy(x_initial_alg3[ell,:])
            
            sol = scipy.optimize.minimize(laplace_convex_con, initial_condition, constraints=con, method='COBYLA',options={'disp': False,'maxiter': 1000})
            
            x_initial_alg3[ell,:] = sol.x
        ...
        alg3_data_k1[sample, counter2] = max(indices_of_true_ranks_1)+1
        ...
        alg3_data_k2[sample, counter2] = max(indices_of_true_ranks_2)+1       
        ...
        alg3_data_k3[sample, counter2] = max(indices_of_true_ranks_3)+1  
        ...
        alg3_data_k4[sample, counter2] = max(indices_of_true_ranks_4)+1   
        ...
        alg3_data_k5[sample, counter2] = max(indices_of_true_ranks_5)+1  
...
# plt.plot(range(number_test_samples_min, number_test_samples_max, number_test_samples_step),1-np.mean(alg3_data <= number_overloads, axis=0), 'r-')
#plt.plot(range(len(np.mean(alg3_data_k1 <= 1, axis=0))),1-np.mean(alg3_data_k2 <= 3, axis=0), 'r-')
np.savetxt('alg3data_k1_G_MC=1000_P=10.csv',alg3_data_k1.astype(int))
np.savetxt('alg3data_k2_G_MC=1000_P=10.csv',alg3_data_k2.astype(int))
#np.savetxt('alg3data_k3_G_MC=1000_P=10.csv',alg3_data_k3.astype(int))
#np.savetxt('alg3data_k4_G_MC=1000_P=10.csv',alg3_data_k4.astype(int))
#np.savetxt('alg3data_k5_G_MC=1000_P=10.csv',alg3_data_k5.astype(int))
