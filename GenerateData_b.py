...
for sample in range(number_MC_samples):
    ...
    for observation in range(number_samples):
        V_samples = np.empty((1, number_generators))
        for i in range(number_generators):
            V_samples[0,i] = np.random.gamma(1, test_beta_values[i], size=1)   
        R_samples = np.random.binomial(1,0.5,(1,number_generators))
        R_samples[R_samples==0] = -1     
        
        network.generators.p_set = np.multiply(V_samples[0], R_samples[0])+nominal_injections
        network.lpf()
        network.pf()
...
np.savetxt('alg1rank_AC_L_MC=1000_ss=1000.csv', indices_of_true_ranks_1.astype(int))
np.savetxt('alg2rank_AC_L_MC=1000_ss=1000.csv', indices_of_true_ranks_2.astype(int))
